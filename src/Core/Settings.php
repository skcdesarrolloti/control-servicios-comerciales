...
final class Settings
{
  ...
  public function __construct(Database $db, bool $readOnly = false, string $functionKey = self::FUNCTION_KEY)
  {
    $this->db = $db;
    $this->table = $db->table('jet_cct_confi_sistema');
    $this->readOnly = $readOnly;
    $this->functionKey = trim($functionKey) !== '' ? trim($functionKey) : self::FUNCTION_KEY;
  }
  ...
  private function decodeSettingsDocument(string $raw): array
  {
    ...
    try {
      ...
      throw new \RuntimeException($this->functionKey . ' contiene JSON inválido.', 0, $exception);
    }
