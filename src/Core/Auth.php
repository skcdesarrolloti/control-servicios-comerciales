...
final class Auth
{
  ...
  public function attempt(string $user, string $pass): bool
  {
    ...
    return $this->establishSession($row, $user);
  }

  /**
   * Inicia sesión para un funcionario activo después de que un mecanismo
   * externo confiable (por ejemplo, un enlace HMAC) validó su identidad.
   */
  public function attemptTrustedUser(string $user): bool
  {
    $user = trim($user);
    if ($user === '') {
      return false;
    ...
    return is_array($row) && $row !== [] && $this->establishSession($row, $user);
  }

  /** @param array<string,mixed> $row */
  private function establishSession(array $row, string $fallbackLogin): bool
  {
    session_regenerate_id(true);
    $_SESSION['scm_logged_in']  = true;
    $_SESSION['scm_user_id']    = (int) ($row['_ID'] ?? 0);
    $_SESSION['scm_employee_id'] = trim((string) ($row['id_empleado'] ?? ''));
    $_SESSION['scm_user']       = trim((string) ($row['nombre'] ?? $fallbackLogin));
    $_SESSION['scm_user_login'] = trim((string) ($row['user_others_apss'] ?? $fallbackLogin));
    $_SESSION['scm_user_rol']   = (string) ($row['rol'] ?? '');
    $_SESSION['scm_user_cargo'] = trim((string) ($row['id_cargo'] ?? ''));

    return $_SESSION['scm_user_id'] > 0;
  }
  ...
  /** Devuelve el id_empleado del funcionario autenticado. */
  public static function employeeId(): string
  {
    return (string) ($_SESSION['scm_employee_id'] ?? '');
  }
